...
void MasterClient::PingServers(void)
{
	...
	{
		rakPeer->Ping((char*)rakPeer->SystemAddressToDottedIP(gameServerList.serverList[serverIndex]->connectionIdentifier),
...
void MasterClient::HandleServerListResponse(Packet *packet, bool overwriteExisting)
{
	...
	for (serverIndex=0; serverIndex < numberOfServers; serverIndex++)
	{
		...
		{
			// Ping the new server
			rakPeer->Ping((char*)rakPeer->SystemAddressToDottedIP(gameServer->connectionIdentifier),
...
void MasterClient::HandleRelayedConnectionNotification(Packet *packet)
{
	...
	OnConnectionRequest(rakPeer->SystemAddressToDottedIP(clientSystem), clientSystem.port);
}
